...
public class MainActivity extends Activity {
    ...
    private void process(){
        ...
        executor.submit(()->{
            ...
            try{
                PowerManager pm=(PowerManager)getSystemService(POWER_SERVICE);lock=pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK,"PodcastForge:Processing");lock.acquire(6*60*60*1000L);
                String stamp=new SimpleDateFormat("yyyyMMdd_HHmmss",Locale.US).format(new Date());ContentValues v=new ContentValues();v.put(MediaStore.Audio.Media.DISPLAY_NAME,"PodcastForge3_"+stamp+".wav");v.put(MediaStore.Audio.Media.MIME_TYPE,"audio/wav");v.put(MediaStore.Audio.Media.RELATIVE_PATH,"Music/PodcastForge");outputUri=getContentResolver().insert(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI,v);if(outputUri==null)throw new Exception("تعذر إنشاء ملف الإخراج");
